...
export async function resolveVendorId(vendorId: string): Promise<VendorResolutionResult> {
  ...
  // Step 2: Convert _id to ObjectId (required for MongoDB queries)
  let vendorObjectId: mongoose.Types.ObjectId
  if (rawVendor._id instanceof mongoose.Types.ObjectId) {
    vendorObjectId = rawVendor._id
    console.log(`[VENDOR_RESOLUTION] vendor._id is already ObjectId`)
  } else if (mongoose.Types.ObjectId.isValid(rawVendor._id)) {
    vendorObjectId = new mongoose.Types.ObjectId(rawVendor._id)
    console.log(`[VENDOR_RESOLUTION] Converted vendor._id from ${typeof rawVendor._id} to ObjectId`)
  } else {
    const error = `Invalid vendor _id format: ${rawVendor._id} (type: ${typeof rawVendor._id})`
    console.error(`[VENDOR_RESOLUTION] ❌ ${error}`)
    throw new Error(error)
  }
  ...
  if (!mongoose.Types.ObjectId.isValid(vendorObjectId)) {
    const error = `Invalid vendorObjectId after conversion: ${vendorObjectId.toString()}`
    console.error(`[VENDOR_RESOLUTION] ❌ ${error}`)
    throw new Error(error)
  }
