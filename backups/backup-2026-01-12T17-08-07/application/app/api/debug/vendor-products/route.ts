  ...
  try {
    ...
    const matchedLinks = allLinks.filter((link: any) => {
      ...
      if (linkVendorId instanceof mongoose.Types.ObjectId) {
        ...
        if ((linkVendorId as any)._id) {
          linkVendorIdObjectId = (linkVendorId as any)._id instanceof mongoose.Types.ObjectId 
            ? (linkVendorId as any)._id 
            : (mongoose.Types.ObjectId.isValid((linkVendorId as any)._id) ? new mongoose.Types.ObjectId((linkVendorId as any)._id) : null)
        } else if (mongoose.Types.ObjectId.isValid(linkVendorId as any)) {
          linkVendorIdObjectId = new mongoose.Types.ObjectId(linkVendorId as any)
        }
      } else {
