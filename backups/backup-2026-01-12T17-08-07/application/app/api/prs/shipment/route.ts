...
export async function POST(request: Request) {
  try {
    ...
    if (!orderDoc) {
      ...
      const orderWithoutVendor: any = await order.findOne({ id: prId }).select('id vendorId companyId').lean()
    ...
    if (orderDoc.companyId) {
      if (typeof orderDoc.companyId === 'object' && orderDoc.companyId?.id) {
        ...
        if (company) {
          ...
          // CRITICAL FIX: companyObjectId could be a STRING ID, not ObjectId
          const companyIdStr = String(companyObjectId)
          const companyByObjectId: any = /^\d{6}$/.test(companyIdStr)
            ? await Company.findOne({ id: companyIdStr }).select('id').lean()
            : await Company.findById(companyObjectId).select('id').lean()
