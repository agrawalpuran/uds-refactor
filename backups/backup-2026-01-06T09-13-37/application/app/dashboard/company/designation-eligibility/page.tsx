...
export default function DesignationEligibilityPage() {
    ...
    try {
      ...
      for (const existing of existingEligibilities) {
        if (!targetSubcategoryIds.has(existing.subcategory?.id)) {
          promises.push(deleteDesignationSubcategoryEligibility(existing._id))
        }
      }
  ...
  const handleDelete = async (eligibility: any) => {
    ...
    try {
      ...
      await Promise.all(
        relatedEligibilities.map((e: any) => deleteDesignationSubcategoryEligibility(e._id))
      )
                      ...
                      <tr key={key} className="hover:bg-gray-50">
                        ...
                        <td className="py-4 px-6 text-gray-600">
                          <div className="space-y-1">
                            {groupEligs.map((elig: any) => (
                              <div key={elig._id} className="text-sm">
