  ...
  try {
    ...
    for (const product of products) {
      ...
      if (subcategoryMapping) {
        ...
        // Get vendor name
        const ProductVendor = mongoose.models.ProductVendor || mongoose.model('ProductVendor', require('@/lib/models/ProductVendor').default)
        const productVendor = await ProductVendor.findOne({ productId: product._id }).populate('vendorId', 'name').lean()
