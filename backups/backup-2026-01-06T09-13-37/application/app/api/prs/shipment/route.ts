...
export async function POST(request: Request) {
  try {
    ...
    const orderDoc = await order.findOne({ id: prId, vendorId }).select('companyId').lean()
    
    if (!orderDoc) {
      // Try to find order without vendorId filter to see if it exists
      const orderWithoutVendor = await order.findOne({ id: prId }).select('id vendorId companyId').lean()
      ...
      if (orderWithoutVendor) {
        ...
        const similarOrders = await order.find({ id: { $regex: prId.substring(0, Math.min(10, prId.length)) } })
          ...
          .limit(5)
          .lean()
        console.error('[API /prs/shipment POST] Similar order IDs found:', similarOrders.map((o: any) => ({ id: o.id, vendorId: o.vendorId })))
    ...
    if (orderDoc.companyId) {
      ...
      if (Buffer.isBuffer(orderDoc.companyId) || orderDoc.companyId instanceof mongoose.default.Types.ObjectId) {
        ...
        const companyObjectId = Buffer.isBuffer(orderDoc.companyId) 
          ...
          : orderDoc.companyId
        const company = await Company.findById(companyObjectId).select('id').lean()
        if (company) {
    ...
    // ====================================================
    const company = await Company.findOne({ id: companyId }).select('shipmentRequestMode name').lean()
    ...
    const shouldUseApiShipment = explicitShipmentMode === 'API' || companyShipmentMode === 'AUTOMATIC'
    ...
    if (companyShipmentMode === 'AUTOMATIC') {
      ...
      if (shipmentData.carrierName && shipmentData.carrierName.trim()) {
        console.log('[API /prs/shipment POST] ⚠️ AUTOMATIC mode: UI provided courier override:', shipmentData.carrierName)
      ...
      if (shipmentData.modeOfTransport && shipmentData.modeOfTransport !== 'COURIER' && shipmentData.modeOfTransport !== 'OTHER') {
        console.log('[API /prs/shipment POST] ⚠️ AUTOMATIC mode: Invalid modeOfTransport:', shipmentData.modeOfTransport)
        console.log('[API /prs/shipment POST] ⚠️ Overriding to COURIER for AUTOMATIC mode')
    ...
    if (shouldUseApiShipment) {
      ...
      try {
        const { getActiveVendorRoutingForCompany } = await import('@/lib/db/vendor-shipping-routing-access')
        const preCheckRouting = await getActiveVendorRoutingForCompany(vendorId, companyId)
        
        if (preCheckRouting) {
          console.log('[API /prs/shipment POST] Found vendor routing, checking if provider is enabled...')
          const ShipmentServiceProvider = await import('@/lib/models/ShipmentServiceProvider').then(m => m.default)
          const preCheckProvider = await ShipmentServiceProvider.findOne({ 
            providerRefId: preCheckRouting.shipmentServiceProviderRefId 
          }).lean()
          
          if (preCheckProvider) {
            const CompanyShippingProvider = await import('@/lib/models/CompanyShippingProvider').then(m => m.default)
            ...
            if (!existingCompanyProvider || !existingCompanyProvider.isEnabled) {
              console.log('[API /prs/shipment POST] ⚠️ Auto-enabling provider from vendor routing...')
        ...
        console.warn('[API /prs/shipment POST] ⚠️ Error during pre-check auto-enablement:', error.message)
      ...
      if (!apiEnabled) {
        ...
        const isAutomaticMode = companyShipmentMode === 'AUTOMATIC' || String(companyShipmentMode).toUpperCase() === 'AUTOMATIC'
        
        console.log('[SHIP-RCA][DECISION_GATE]', {
          companyShippingMode: companyShipmentMode,
          enabledProvidersCount: enabledProviders.length,
          hasVendorRouting: !!errorCheckRouting,
          hasCredentials: enabledProviders.some((p: any) => p.apiKey || p.apiSecret || p.accessToken),
          reasonForFallback: !isAutomaticMode ? 'Company mode is not AUTOMATIC' : enabledProviders.length === 0 ? 'No enabled providers' : 'Unknown',
        ...
        if (isAutomaticMode) {
          ...
          console.error('[API /prs/shipment POST] 🚫 THROWING ERROR - AUTOMATIC mode requires providers:', errorMessage)
        ...
        if (explicitShipmentMode === 'API') {
          console.error('[API /prs/shipment POST] 🚫 THROWING ERROR - Explicit API mode requires providers')
          return NextResponse.json(
            { 
        ...
        console.log('[SHIP-RCA][DECISION_GATE] ⚠️ FALLING BACK TO MANUAL - This should NOT happen if companyShipmentMode is AUTOMATIC', {
          companyShipmentMode,
          isAutomaticMode,
          explicitShipmentMode,
          enabledProvidersCount: enabledProviders.length,
        ...
        console.log('[API /prs/shipment POST] ⚠️ API shipment not enabled, falling back to manual (backward compatibility mode)')
        ...
        if (!providerId || !companyShippingProviderId) {
          console.log('[API /prs/shipment POST] Provider info not in request, fetching from vendor routing...')
          const { getActiveVendorRoutingForCompany } = await import('@/lib/db/vendor-shipping-routing-access')
          ...
          if (routing) {
            ...
            if (provider) {
              ...
              // ====================================================
              const CompanyShippingProvider = await import('@/lib/models/CompanyShippingProvider').then(m => m.default)
              let companyProvider = await CompanyShippingProvider.findOne({
                companyId,
                providerId: provider.providerId,
              ...
              console.log('[SHIP-RCA][CREDENTIAL_CHECK]', {
                ...
                hasCredentials: companyProvider ? !!(companyProvider.apiKey || companyProvider.apiSecret || companyProvider.accessToken) : false,
              ...
              if (!companyProvider) {
                console.log('[API /prs/shipment POST] ⚠️ CompanyShippingProvider not found, auto-enabling provider from vendor routing...')
                const { generateShippingId } = await import('@/lib/db/shipping-config-access')
                const newCompanyShippingProviderId = generateShippingId('CSP')
              ...
              if (companyProvider) {
                ...
                console.log('[API /prs/shipment POST] Company shipping provider mapping not found for provider:', provider.providerId)
              }
            } else {
              console.log('[API /prs/shipment POST] Provider not found for refId:', routing.shipmentServiceProviderRefId)
            }
          } else {
            console.log('[API /prs/shipment POST] No active vendor shipping routing found for vendor:', vendorId, 'company:', companyId)
          }
        }
        ...
        if (!providerId || !companyShippingProviderId) {
          ...
          if (companyShipmentMode === 'AUTOMATIC') {
            return NextResponse.json(
              { 
                error: `Unable to resolve shipping provider for company ${company?.name || companyId}. Please ensure vendor shipping routing is configured correctly.`,
          ...
          console.error('[SHIP-RCA][DECISION_GATE] ⚠️ CRITICAL: Falling back to MANUAL despite missing provider info', {
            companyShipmentMode,
            hasProviderId: !!providerId,
            hasCompanyShippingProviderId: !!companyShippingProviderId,
            reason: 'Provider info missing but not throwing error (backward compatibility)',
          })
          ...
          console.log('[SHIP-RCA][FINAL_CONTEXT]', {
            ...
            courierSelected: shipmentData.vendorRouting?.primaryCourierCode || shipmentData.selectedCourierType || 'N/A',
          ...
          if (!apiResult.success) {
            ...
            if (companyShipmentMode === 'AUTOMATIC') {
              console.error('[SHIP-RCA][DECISION_GATE] 🚫 CRITICAL: API shipment failed in AUTOMATIC mode', {
                companyShipmentMode,
                error: apiResult.error,
                reason: 'API shipment creation failed - cannot fall back to MANUAL in AUTOMATIC mode',
                providerId,
                companyShippingProviderId,
              })
              ...
              return NextResponse.json(
                {
                  error: `Automatic shipment creation failed: ${apiResult.error || 'Unknown error'}. Please check provider credentials and configuration.`,
                  ...
                  details: {
                    ...
                    companyShippingProviderId,
                    error: apiResult.error,
                    suggestion: apiResult.error?.includes('email') || apiResult.error?.includes('password') 
            ...
            if (shipmentData.allowManualFallback !== false) {
              console.log('[API /prs/shipment POST] API shipment failed, falling back to manual:', apiResult.error)
              // Continue with manual shipment below
            } else {
              return NextResponse.json(
            ...
            console.log('[API /prs/shipment POST] 🔑 CRITICAL: Passing shipmentId from createApiShipment as shipmentReferenceNumber')
            console.log(`[API /prs/shipment POST]    ShipmentId from createApiShipment: ${apiResult.shipmentId}`)
            ...
            console.log('[API /prs/shipment POST] ✅ PR updated successfully with API shipment')
            console.log(`[API /prs/shipment POST]    Updated PR shipmentId: ${updatedPR.shipmentId}`)
    ...
    console.log('[API /prs/shipment POST] Using MANUAL shipment mode', {
      ...
      companyName: company?.name,
      companyShipmentMode,
      reason: shouldUseApiShipment ? 'API shipment was requested but fell back to manual' : 'Manual shipment mode (default or explicit)',
    })
