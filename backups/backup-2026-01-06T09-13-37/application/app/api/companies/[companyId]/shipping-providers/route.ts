...
export async function POST(
  ...
  try {
    ...
    const existing = await CompanyShippingProvider.findOne({
      companyId,
      providerId,
    }).lean()
...
export async function PUT(
  ...
  try {
    ...
    const updated = await CompanyShippingProvider.findOneAndUpdate(
      { companyId, providerId },
      { $set: updateData },
      { new: true }
    ).lean()

    console.log(`[PUT /api/companies/${companyId}/shipping-providers] ✅ Updated CompanyShippingProvider: ${existing.companyShippingProviderId}`)

    // Get provider details
    const provider = await ShipmentServiceProvider.findOne({ providerId }).select('providerCode providerName').lean()
