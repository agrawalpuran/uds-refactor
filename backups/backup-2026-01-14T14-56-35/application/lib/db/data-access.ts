...
export async function getProductsByVendor(vendorId: string): Promise<any[]> {
  ...
  // CRITICAL: VendorInventory stores productId as ObjectId, so we need to convert product string IDs to ObjectIds
  // First, get products to get their _id ObjectIds
  const productObjectIds: mongoose.Types.ObjectId[] = []
  for (const productIdStr of productStringIds) {
    const product = await Uniform.findOne({ id: productIdStr }).lean()
  ...
  const inventoryRecords = await VendorInventory.find({
    vendorId: vendorIdStr,
    productId: { $in: productObjectIds }, // CRITICAL: Only inventory for assigned products (using ObjectId for VendorInventory)
  })
    .lean()
  ...
  // Create a map of product ObjectId -> product string ID for validation
  const productObjectIdToStrMap = new Map<string, string>()
  for (const productIdStr of productStringIds) {
    const product = await Uniform.findOne({ id: productIdStr }).lean()
    if (product && product._id) {
      const productObjectIdStr = product._id instanceof mongoose.Types.ObjectId
        ? product._id.toString()
        : String(product._id)
      productObjectIdToStrMap.set(productObjectIdStr, productIdStr)
    }
  }
  
  const validProductObjectIdStrs = new Set(productObjectIdToStrMap.keys())
  const invalidInventory = inventoryRecords.filter((inv: any) => {
    const invProductIdStr = inv.productId?.toString() || String(inv.productId || '')
    return !validProductObjectIdStrs.has(invProductIdStr)
  })
  ...
  validInventoryRecords.forEach((inv: any) => {
    const invProductIdStr = inv.productId?.toString() || String(inv.productId || '')
    const productIdStr = productObjectIdToStrMap.get(invProductIdStr)
    if (productIdStr) {
...
export async function createPurchaseOrderFromPRs(
  ...
  for (const [vendorNumericId, vendorOrders] of ordersByVendor.entries()) {
    ...
    for (const orderDoc of vendorOrders) {
      ...
      // Get order _id for update query (MongoDB still uses _id for updates)
      ...
      if (orderDoc._id) {
        ...
        // Fetch order to get _id for update query
        const orderForId = await Order.findOne({ id: orderIdString })
        if (!orderForId) {
          throw new Error(`Order not found: ${orderIdString}`)
        }
      ...
      // Update order status - use updateOne with _id (MongoDB still uses _id for updates)
