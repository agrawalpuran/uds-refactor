...
export default function EmployeesPage() {
              ...
              companyLocations.forEach((loc: any) => {
                if (loc.adminId) {
                  ...
                  if (typeof loc.adminId === 'object') {
                    ...
                    if (adminStringId && /^\d{6}$/.test(String(adminStringId))) {
  ...
  const isSiteAdmin = (employee: any): boolean => {
    ...
    if (employee.id && /^\d{6}$/.test(String(employee.id))) {
    ...
    if (!isAdmin && employee.employeeId && /^\d{6}$/.test(String(employee.employeeId))) {
