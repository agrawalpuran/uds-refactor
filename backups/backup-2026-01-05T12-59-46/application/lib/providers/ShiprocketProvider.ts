...
export class ShiprocketProvider implements LogisticsProvider {
  ...
  async createShipment(payload: CreateShipmentPayload): Promise<CreateShipmentResult> {
    ...
    try {
      ...
      // Extract shipment_id from response (required for AWB assignment)
      const shipmentId = createResponse.shipment_id || createResponse.shipmentId
      if (!shipmentId) {
        console.error('[ShiprocketProvider] ❌ shipment_id not found in create response:', createResponse)
        throw new Error('Shipment ID not received from Shiprocket. Cannot assign AWB.')
      }
      ...
      // Step 3: ALWAYS fetch order details using order_id to get AWB (more reliable)
      // AWB may not be immediately available, so we retry with delays
      const orderId = createResponse.order_id?.toString() || shipmentId?.toString()
      if (orderId) {
      ...
      return {
        success: true,
        providerShipmentReference: createResponse.order_id?.toString() || shipmentId?.toString() || '',
        ...
        rawResponse: {
          createResponse,
          awbAssignResponse,
          orderDetailsResponse, // Include order details response for debugging
        },
      }
    } catch (error: any) {
      return {
      ...
      try {
        ...
        try {
          ...
          if (errorMsg.includes('not found') || errorMsg.includes('404') || errorMsg.includes('Invalid')) {
            throw new Error(`Docket number "${docketNumber}" not found. Please verify the docket number is correct.`)
          }
