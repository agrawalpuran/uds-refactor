...
export async function createProvider(
  ...
  if (companyId && companyShippingProviderId) {
    const companyProvider = await CompanyShippingProvider.findOne({
    ...
    if (providerInstance instanceof ShipwayProvider) {
      ...
      // Shiprocket requires authentication, so we can't initialize without credentials
      throw new Error('ShiprocketProvider requires email and password for initialization')
    } else if (providerInstance.constructor.name === 'MockProvider') {
