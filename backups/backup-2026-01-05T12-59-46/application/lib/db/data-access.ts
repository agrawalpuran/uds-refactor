...
async function convertCompanyIdToNumericId(companyIdObjectId: any): Promise<string | null> {
  if (!companyIdObjectId) return null
  ...
  try {
    ...
    if (/^[0-9a-fA-F]{24}$/.test(companyIdStr)) {
      const companyDoc = await db.collection('companies').findOne({
        _id: companyIdObjectId instanceof mongoose.Types.ObjectId 
          ? companyIdObjectId 
    ...
    console.warn(`[convertCompanyIdToNumericId] Error converting companyId:`, error)
    return null
  }
}
...
export async function getOrdersByVendor(vendorId: string): Promise<any[]> {
  ...
  console.log(`[getOrdersByVendor] Found ${orders.length} order(s) with vendorId=${vendorNumericId}`)
  ...
  if (orders.length === 0 && mongoose.Types.ObjectId.isValid(vendorNumericId) === false) {
    ...
    const legacyOrders = await Order.find({ vendorId: vendor._id })
      .select('id employeeId employeeName items total status orderDate dispatchLocation companyId deliveryAddress parentOrderId vendorId vendorName isPersonalPayment personalPaymentAmount createdAt pr_number pr_date pr_status deliveryStatus dispatchStatus shipping_address_line_1 shipping_address_line_2 shipping_address_line_3 shipping_city shipping_state shipping_pincode shipping_country')
      .populate('employeeId', 'id firstName lastName email locationId')
      .populate('companyId', 'id name')
      .populate('items.uniformId', 'id name')
    ...
    if (legacyOrders.length > 0) {
      console.log(`[getOrdersByVendor] Found ${legacyOrders.length} legacy order(s) with ObjectId vendorId`)
...
export async function approveReturnRequest(
  ...
  // CRITICAL: This function only extracts vendorId - no fallbacks, no name lookups
  const extractVendorId = (order: any, orderLabel: string = 'order'): { vendorId: mongoose.Types.ObjectId | null; vendorName: string | null } => {
    ...
    // Try populated vendorId (object with _id and name)
    if (order.vendorId) {
      if (order.vendorId instanceof mongoose.Types.ObjectId) {
        vendorId = order.vendorId
        vendorName = order.vendorName || null
        console.log(`[approveReturnRequest] ✅ Extracted vendorId as ObjectId: ${vendorId}`)
      } else if (typeof order.vendorId === 'object' && order.vendorId !== null) {
        // Populated vendor object - extract _id (ObjectId)
        if (order.vendorId._id) {
          vendorId = order.vendorId._id instanceof mongoose.Types.ObjectId
            ? order.vendorId._id
            : new mongoose.Types.ObjectId(order.vendorId._id.toString())
          vendorName = order.vendorId.name || order.vendorName || null
          console.log(`[approveReturnRequest] ✅ Extracted vendorId from populated object _id: ${vendorId}, name: ${vendorName}`)
        } else {
          // Check if the object itself is an ObjectId-like structure
          // Sometimes Mongoose returns the ObjectId directly in the object
          const keys = Object.keys(order.vendorId)
          console.warn(`[approveReturnRequest] ⚠️ Populated vendorId object missing _id field. Keys:`, keys, `Full object:`, order.vendorId)
          
          // Try to find ObjectId in the object
          for (const key of keys) {
            if (order.vendorId[key] instanceof mongoose.Types.ObjectId) {
              vendorId = order.vendorId[key]
              vendorName = order.vendorId.name || order.vendorName || null
              console.log(`[approveReturnRequest] ✅ Found ObjectId in key "${key}": ${vendorId}`)
              break
            }
          }
        }
        ...
        try {
          if (mongoose.Types.ObjectId.isValid(order.vendorId)) {
            vendorId = new mongoose.Types.ObjectId(order.vendorId)
            vendorName = order.vendorName || null
            console.log(`[approveReturnRequest] ✅ Converted vendorId string to ObjectId: ${vendorId}`)
          } else {
            console.warn(`[approveReturnRequest] ⚠️ Invalid vendorId string (not a valid ObjectId): ${order.vendorId}`)
          }
        } catch (e) {
          console.warn(`[approveReturnRequest] ⚠️ Error converting vendorId string to ObjectId: ${order.vendorId}`, e)
        }
      }
    } else {
  ...
  if (isParentOrder) {
    ...
    for (const childOrder of childOrdersWithVendor) {
      ...
      if (returnRequest.originalOrderItemIndex >= currentItemIndex && 
        ...
        // Found the child order containing this item
        const vendorInfo = extractVendorId(childOrder, `child order ${childOrder.id}`)
    ...
    const vendorInfo = extractVendorId(originalOrder, 'original order')
    originalVendorId = vendorInfo.vendorId
    originalVendorName = vendorInfo.vendorName
    ...
    if (!originalVendorId && originalOrderRaw.vendorId) {
      ...
      if (originalOrderRaw.vendorId instanceof mongoose.Types.ObjectId) {
      ...
      } else if (typeof originalOrderRaw.vendorId === 'string' && mongoose.Types.ObjectId.isValid(originalOrderRaw.vendorId)) {
        originalVendorId = new mongoose.Types.ObjectId(originalOrderRaw.vendorId)
        originalVendorName = originalOrderRaw.vendorName || null
        console.log(`[approveReturnRequest] ✅ Converted raw order vendorId string to ObjectId: ${originalVendorId}`)
      }
    }
  ...
  // CRITICAL: Validate that vendorId is a valid ObjectId
  if (!(originalVendorId instanceof mongoose.Types.ObjectId)) {
    console.error(`[approveReturnRequest] ❌ CRITICAL: vendorId is not a valid ObjectId:`, {
      vendorId: originalVendorId,
      type: typeof originalVendorId,
      constructor: originalVendorId?.constructor?.name
    })
    throw new Error(`Invalid vendorId format: ${originalVendorId}. Expected MongoDB ObjectId. Order ID: ${returnRequest.originalOrderId}. Cannot create replacement order.`)
  }
  
  // CRITICAL: Validate that vendor exists in database
  // Try multiple query methods to ensure we find the vendor
  console.log(`[approveReturnRequest] 🔍 DEBUG: Querying vendor with ObjectId:`, {
    vendorId: originalVendorId.toString(),
    vendorIdHex: originalVendorId.toHexString(),
    isValid: mongoose.Types.ObjectId.isValid(originalVendorId),
    vendorIdConstructor: originalVendorId.constructor.name
  })
  
  // Try Mongoose query first
  let vendorExists = await Vendor.findById(originalVendorId).lean()
  
  // If not found, try raw collection query (bypasses Mongoose)
  if (!vendorExists) {
    console.warn(`[approveReturnRequest] ⚠️ Vendor not found by Mongoose findById, trying raw collection query...`)
    const db = mongoose.connection.db
    if (db) {
      // Try with ObjectId directly
      const rawVendor = await db.collection('vendors').findOne({ _id: originalVendorId })
      
      // If still not found, try with string conversion
      if (!rawVendor) {
        const vendorIdStr = originalVendorId.toString()
        const rawVendorByStr = await db.collection('vendors').findOne({ _id: new mongoose.Types.ObjectId(vendorIdStr) })
        if (rawVendorByStr) {
          console.log(`[approveReturnRequest] ✅ Found vendor using string-converted ObjectId`)
          vendorExists = rawVendorByStr as any
        }
      } else {
        console.log(`[approveReturnRequest] ✅ Found vendor in raw collection:`, {
          _id: rawVendor._id?.toString(),
          id: rawVendor.id,
          name: rawVendor.name
        })
        vendorExists = rawVendor as any
      }
      
      // If still not found, list sample vendors for debugging
      if (!vendorExists) {
        const sampleVendors = await db.collection('vendors').find({}).limit(10).toArray()
        console.error(`[approveReturnRequest] ❌ Vendor not found. Sample vendors in database:`, 
          sampleVendors.map((v: any) => ({ 
            _id: v._id?.toString(), 
            _idType: v._id?.constructor?.name,
            id: v.id, 
            name: v.name 
          }))
        )
        
        // Check if any vendor has a matching _id when converted to string
        const matchingVendor = sampleVendors.find((v: any) => {
          if (!v._id) return false
          const vIdStr = v._id.toString()
          const vIdHex = v._id.toHexString ? v._id.toHexString() : vIdStr
          return vIdStr === originalVendorId.toString() || vIdHex === originalVendorId.toHexString()
        })
        
        if (matchingVendor) {
          console.log(`[approveReturnRequest] ✅ Found matching vendor by string comparison:`, matchingVendor)
          vendorExists = matchingVendor as any
        }
      }
    }
  }
  ...
  if (!vendorExists) {
    const errorDetails = {
      ...
      vendorId: originalVendorId.toString(),
      vendorIdHex: originalVendorId.toHexString(),
      vendorIdType: originalVendorId.constructor.name,
      isValidObjectId: mongoose.Types.ObjectId.isValid(originalVendorId),
      rawOrderVendorId: originalOrderRaw.vendorId?.toString(),
      populatedOrderVendorId: originalOrder.vendorId?.toString()
    }
    console.error(`[approveReturnRequest] ❌ CRITICAL: Vendor not found in database after all attempts:`, errorDetails)
    throw new Error(`Vendor not found: ${originalVendorId.toString()}. The vendor may have been deleted or the vendorId in the order is invalid. Order ID: ${returnRequest.originalOrderId}. Cannot create replacement order.`)
  }
