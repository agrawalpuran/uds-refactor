...
export async function createApiShipment(
  ...
  try {
    ...
    console.log(`[createApiShipment] 📦 Creating Shipment document with shipmentId: ${shipmentId} for PR: ${prId}`)
    ...
    console.log(`[createApiShipment] ✅ Shipment document created successfully: ${shipmentId}`)
...
export async function fetchShipmentAWB(shipmentId: string): Promise<{ success: boolean; awbNumber?: string; error?: string }> {
  ...
  try {
    ...
    console.log(`[fetchShipmentAWB] Fetching AWB for shipment: ${shipmentId}`)
    ...
    if (!shipment.providerShipmentReference) {
      throw new Error('Provider shipment reference is missing. Cannot fetch AWB.')
    }
