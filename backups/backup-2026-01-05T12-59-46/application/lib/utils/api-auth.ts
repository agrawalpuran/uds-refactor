...
export async function getUserEmailFromRequest(request: NextRequest): Promise<string | null> {
  ...
  // Method 2: Try request body (need to clone request to read body multiple times)
  try {
  ...
  // Method 3: Try authorization header (Bearer token with email)
  const authHeader = request.headers.get('authorization')
  ...
  // Method 4: Try X-User-Email header (custom header for API calls)
