...
import { 
  ...
  Settings, LogOut, MapPin, ShoppingCart, Upload, Shield, Warehouse, MessageSquare, Menu, X, RefreshCw, Layers, Tag, Link2, ChevronDown, ChevronRight, Truck
} from 'lucide-react'
...
export default function DashboardLayout({ children, actorType }: DashboardLayoutProps) {
  ...
  useEffect(() => {
    if (actorType === 'company' && isCompanyAdmin) {
      companyAdminMenuCategories.forEach(category => {
        ...
        <nav className="mt-6 px-3">
          {menu.map((item) => {
            ...
            if ('items' in item && Array.isArray(item.items)) {
              ...
              // Calculate total badge count for this category
              ...
              category.items.forEach((subItem) => {
                if (subItem.name === 'Return Requests' || subItem.href === '/dashboard/company/returns') {
                  ...
                  // Badge for GRN/Invoice status changes - show if there are pending GRNs or invoices
                  // This is a simple indicator, can be enhanced later with actual counts
                  categoryBadgeCount += 0 // Placeholder - can be enhanced with actual GRN/Invoice status counts
                }
              })
                    ...
                    <div className="ml-4 mt-1 space-y-1">
                      {category.items.map((subItem) => {
                        ...
                        if (subItem.name === 'Return Requests' || subItem.href === '/dashboard/company/returns') {
                          ...
                          // Badge for GRN/Invoice - show a dot indicator for status changes
                          // This is a simple visual indicator, can be enhanced with actual counts later
                          badgeCount = 0 // Placeholder - can be enhanced with actual GRN/Invoice status counts
                        }
