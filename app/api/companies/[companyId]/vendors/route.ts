...
export async function GET(
  ...
  try {
    ...
    if (vendorCompanyLinks.length === 0) {
      ...
      const productIds = productCompanyLinks.map(pc => String(pc.productId || pc.uniformId)).filter(Boolean)
      ...
      const vendorIds = Array.from(new Set(productVendorLinks.map(pv => String(pv.vendorId)).filter(Boolean)))
    ...
    const vendorIds = vendorCompanyLinks.map(vc => String(vc.vendorId)).filter(Boolean)
